var help = {
	options: {
		ACTIVE_COLOR: "#e93b3d",
		NORMAL_COLOR: "#333",
		subpages: ["html/sort.html", "html/shop.html", "html/my.html"]
	},
	...
	toggleNview: function(currIndex) {
		...
		help.updateSubNView(currIndex, help.options.ACTIVE_COLOR);
		help.updateSubNView(currIndex + 1, help.options.ACTIVE_COLOR);
		// 重绘兄弟tag 反之排除当前点击的icon和text
		for (var i = 0; i < 8; i++) {
			if (i !== currIndex && i !== currIndex + 1) {
				help.updateSubNView(i, help.options.NORMAL_COLOR);
			}
		}
	},
	/*
	...
	updateSubNView: function(currIndex, color) {
		...
		nviewEvent.drawText(currTag.text, currTag.position, help.changeColor(currTag.textStyles, color), currTag.id);
	}
}
